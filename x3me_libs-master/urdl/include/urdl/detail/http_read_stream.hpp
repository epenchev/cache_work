...
public:
    ...
    http_read_stream(boost::asio::io_service& io_service, option_set& options,
                     Arg& arg)
        : resolver_(io_service)
    ...
    boost::system::error_code open(const url& u, boost::system::error_code& ec)
    {
        ...
        const auto& bind_point =
            options_.get_option<urdl::http::bind_point>().value();
        // Establish a connection to the HTTP server.
        connect(socket_.lowest_layer(), resolver_, bind_point, u, ec);
        if (ec)
            return ec;
        ...
        request_stream << u.to_string(url::path_component |
                                      url::query_component);
        request_stream << " HTTP/1.0\r\n";
        request_stream << "Host: ";
        request_stream << u.to_string(url::host_component |
                                      url::port_component);
        request_stream << "\r\n";
        ...
        for (;;)
        {
            ...
            if (!parse_http_status_line(
                    ...
                    std::istreambuf_iterator<char>(), version_major,
                    version_minor, status_code))
            {
        ...
        if (!parse_http_headers(headers_.begin(), headers_.end(), content_type_,
                                content_length_, location_))
        {
    ...
    public:
                        ...
                        std::size_t bytes_transferred = 0)
        {
            ...
            // Establish a connection to the HTTP server.
            URDL_CORO_YIELD(async_connect(
                socket_.lowest_layer(), resolver_,
                options_.get_option<urdl::http::bind_point>().value(), url_,
                *this));
            if (ec)
            {
                handler_(ec);
...

            {
                ...
                // so that
                // the server will close the socket after transmitting the
                // response.
                // This will allow us to treat all data up until the EOF as the
                // content.
                std::ostream request_stream(&request_buffer_);
                request_stream << request_method << " ";
                request_stream << url_.to_string(url::path_component |
                                                 url::query_component);
                request_stream << " HTTP/1.0\r\n";
                request_stream << "Host: ";
                request_stream << url_.to_string(url::host_component |
                                                 url::port_component);
                request_stream << "\r\n";
                request_stream << "Accept: */*\r\n";
                if (request_content.length())
            ...
            // Send the request.
            URDL_CORO_YIELD(boost::asio::async_write(
                socket_, request_buffer_, boost::asio::transfer_all(), *this));
            if (ec)
            {
                handler_(ec);
                return;
            ...
            {
                // Read the reply status line.
                URDL_CORO_YIELD(boost::asio::async_read_until(
            ...
            // Read list of headers and save them. If there's anything left in
            // the
            // reply buffer afterwards, it's the start of the content returned
            // by the
            // HTTP server.
            URDL_CORO_YIELD(boost::asio::async_read_until(
                socket_, reply_buffer_, "\r\n\r\n", *this));
            ...
            if (status_code_ != http::errc::ok)
                ec = make_error_code(
                    static_cast<http::errc::errc_t>(status_code_));
    ...
    {
        open_coro<Handler>(handler, resolver_, socket_, options_,
                           request_buffer_, reply_buffer_, u, headers_,
                           content_type_, content_length_,
                           location_)(boost::system::error_code(), 0);
    }
    ...
    {
    public:
        read_handler(Handler handler) : handler_(handler) {}
