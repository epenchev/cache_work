...
public:
    ...
    /**
     * @param options The options to be set on the stream. The options in the
     * set
     ...
     * @endcode
     */
    option_set get_options() const { return options_; }
    ...
    boost::system::error_code open(const url& u, boost::system::error_code& ec)
    {
        ...
        for (;;)
        {
            ...
            else if (tmp_url.protocol() == "http")
            {
                ...
                if (ec == http::errc::moved_permanently ||
                    ec == http::errc::found)
                {
                    std::size_t max_redirects =
                        options_.get_option<urdl::http::max_redirects>()
                            .value();
     ...
     * @param u The URL to open.
     *
     * @param handler The handler to be called when the open operation
     * completes.
    ...
    /**
     * @returns A string containing the headers returned with the content from
     * the
     ...
     * type must meet the requirements for @c MutableBufferSequence, as defined
     * in
     ...
     * will
     * block until one or more bytes of data has been read successfully, or
     * until
     * an error occurs.
     *
     * The @c read_some operation may not read all of the requested number of
     ...
     * type must meet the requirements for @c MutableBufferSequence, as defined
     * in
     * the Boost.Asio documentation. Although the buffers object may be copied
     * as
...
private:
    ...
    public:
        open_coro(read_stream* this_ptr, const url& u, Handler handler)
            : this_(this_ptr), url_(u), handler_(handler)
        {
        }
    ...
    detail::http_read_stream<
        boost::asio::ssl::stream<boost::asio::ip::tcp::socket>> https_;
#endif // !defined(URDL_DISABLE_SSL)
    enum
    {
        unknown,
