...
istreambuf::istreambuf() : body_(new body)
{
    init_buffers();
}
...
std::streambuf::int_type istreambuf::underflow()
{
    if (gptr() == egptr())
    {
        ...
        detail::istreambuf_read_handler rh = {body_->error_, bytes_transferred,
                                              body_->timer_};
        body_->read_stream_.async_read_some(
            boost::asio::buffer(boost::asio::buffer(body_->get_buffer_) +
                                body::putback_max),
            rh);
